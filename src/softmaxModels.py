...
class Softmax:
	...
	def pointEval(self,softClass,point):
	...
	def plot3D(self,low=[-5,-5,-5],high=[5,5,5]):
		...
		for clas in range(1,self.size):
			...
			for i in range(0,21):
				for j in range(0,21):
					for k in range(0,21):
						data[i][j][k] = self.pointEval(clas,[(i-10)/2,(j-10)/2,(k-10)/2]);
						if(data[i][j][k] > 0.1):
...
def testLogisticRegression():
	...
	testPoint = [1,2]; 
	winPercent = a.pointEval(1,testPoint); 
	lossPercent = a.pointEval(0,testPoint); 
